...
class SQLiteBase(SQLBase):
    ...
    def _init(self):
        ...
        self._conn = self._new_db_connection(
            self.path, self.multithreading, self.timeout, self.passwd
        )
        ...
        self._conn.execute(f'PRAGMA key="{self.passwd}";')
    ...
    def _new_db_connection(self, path, multithreading, timeout, passwd):
        conn = None
        if path == self._MEMORY:
            conn = sqlcipher.connect(path, check_same_thread=not multithreading)
        else:
        ...
        conn.execute('PRAGMA key="{self.passwd}";')
