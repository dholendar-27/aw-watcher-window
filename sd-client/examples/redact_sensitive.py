...
def main():
    ...
    global sd
    sd = ActivityWatchClient(testing=True)
    ...
    if bid_to_redact == "*":
        for bucket_id in buckets.keys():
            if bucket_id.startswith("sd-watcher-afk"):
                continue
...
def _redact_bucket(bucket_id: str, pattern: Union[str, Pattern]):
    ...
    global sd
    events = sd.get_events(bucket_id, limit=-1)
    ...
    if yes_redact == "y":
        for e in events:
            if e.id in sensitive_ids:
                ...
                if DRYRUN:
                    print("DRYRUN, would do: sd.insert_event(bucket_id, e)")
                else:
                    sd.delete_event(bucket_id, cast(int, e_before.id))
                    sd.insert_event(bucket_id, e)
