...
import sd_client
from sd_transform.filter_period_intersect import _intersecting_eventpairs
...
def main():
    ...
    sd = sd_client.ActivityWatchClient(testing=True)

    buckets = sd.get_buckets()
    ...
    src_events = sd.get_events(src_id)
    print(f"✓ src events: {len(src_events)}")
    dest_events = sd.get_events(dest_id)
    ...
    sd.insert_events(dest_id, src_events)

    print("Operation complete")
    if input("Do you want to delete the source bucket? (y/n): ") == "y":
        sd.delete_bucket(src_id, force=True)
