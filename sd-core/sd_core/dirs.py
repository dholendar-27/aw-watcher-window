...
def get_data_dir(module_name: Optional[str] = None) -> str:
    """
     Get the TTim data directory. If module_name is specified return the path to that module otherwise return the full path to the user's data directory.
     
     @param module_name - The name of the module to get the path to.
     
     @return The path to the TTim data directory or the full path to the module's data directory
    """
    data_dir = platformdirs.user_data_dir("TTim")
...
def get_cache_dir(module_name: Optional[str] = None) -> str:
    """
     Get the cache directory for TTim. If module_name is specified it will be appended to the cache directory.
     
     @param module_name - The name of the module that is going to be cached.
     
     @return The path to the cache directory for TTim or the path to the module itself if no module name
    """
    cache_dir = platformdirs.user_cache_dir("TTim")
...
def get_config_dir(module_name: Optional[str] = None) -> str:
    """
     Get the TTim config directory. If module_name is specified return the path to that module otherwise return the path to the user's config directory.
     
     @param module_name - The name of the module to get the path to.
     
     @return The path to the TTim config directory or the user's config directory if module_name is None
    """
    config_dir = platformdirs.user_config_dir("TTim")
...
def get_log_dir(module_name: Optional[str] = None) -> str:  # pragma: no cover
    """
     Get the path to TTim's log directory. If module_name is specified it will be appended to the log directory to form a fully qualified path
    ...
    # https://github.com/TTim/sd-core/pull/122#issuecomment-1768020335
    # Return the path to the log directory for the current user s log files.
    if sys.platform.startswith("linux"):
        log_dir = platformdirs.user_cache_path("TTim") / "log"
    else:
        log_dir = platformdirs.user_log_dir("TTim")
