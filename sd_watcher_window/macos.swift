...
func logPrefix(_ level: String) -> String {
  return "\(logTimestamp()) [sd-watcher-window-macos] [\(level)]"
}
...
func sendHeartbeat(_ heartbeat: Heartbeat) {
  ...
  Task {
    if oldPayloadDifferent {
      ...
      do {
        // unlike the python sd-client library, we do not enforce a `commit_interval` and instead send the old event (which is not invalid)
        // at the current time with a pulse value equal to the time since this event was originally sent. The sd-server will then merge
        ...
        let refreshedOldHeartbeat = Heartbeat(
          ...
          // more info: https://github.com/ActivityWatch/sd-watcher-window/pull/69
          // we don't *think* this millisecond subtraction is necessary, but it may be:
          // https://github.com/ActivityWatch/sd-watcher-window/pull/69#discussion_r987064282
