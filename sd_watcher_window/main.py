...
def main():
    """
     Main function for sd - watcher. Creates a bucket and waits for it to start. This is a blocking call
    """
    args = parse_args()
    ...
    client = ActivityWatchClient(
        "sd-watcher-window", host=args.host, port=args.port, testing=args.testing
    )
    ...
    logger.info("sd-watcher-window started")
    ...
    with client:
        ...
        if sys.platform == "darwin" and args.strategy == "swift":
            ...
            binpath = os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "sd-watcher-window-macos"
            )
